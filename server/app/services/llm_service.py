...
Format: Question[QUESTION_END]Answer[ANSWER_END]"""
    ...
    def generate_quiz(self, cards: List[Dict[str, str]]) -> Dict[str, any]:
        ...
        context = " ".join([f"Q: {c['front']} A: {c['back']}" for c in cards])
        
        prompt = f"""Based on the following Q&A pairs, create a multiple choice question.
...
C) [Option C]
D) [Option D]
Correct Answer: [The letter of the correct answer, e.g. A]
"""

        try:
            results = LLMService._pipeline(prompt)
            raw_output = results[0]['generated_text']
            print(f"🤖 [QUIZ RAW]: {raw_output[:200]}...")
            
            return self._parse_quiz(raw_output)
        except Exception as e:
